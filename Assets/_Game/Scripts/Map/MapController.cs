...
namespace FantasyGuildmaster.Map
{
    public sealed class MapController : MonoBehaviour
    {
        ...
        private void EnterGuildHallEvening()
        {
            ...
            _endDayRequested = false;
            EnsureEventSystem();
            guildHallPanel.ShowEvening(_guildHallEveningData, _dayIndex, AdvanceFromGuildHallToNextDay, ApplyGuildHallRestEveningEffect);
        }

        private void ApplyGuildHallRestEveningEffect()
        {
            var squads = GetRosterSquads();
            for (var i = 0; i < squads.Count; i++)
            {
        ...
        private void AdvanceFromGuildHallToNextDay()
        {
            guildHallPanel?.Hide();
            _endDayRequested = false;
            _dayIndex++;
            ...
            RefreshContractsAfterDayAdvance();
            SyncAllContractIcons();
        ...
        private void RefreshContractsAfterDayAdvance()
        {
            foreach (var region in _gameData.regions)
            {
                if (region == null || region.id == GuildHqId)
        ...
        #region GuildHall DayFlow (Single Source of Truth)
